...
class MAIOMessage:
        ...
        while True:
            ...
            if bytes_total[pos:pos+4] == b'\x01\x02\x03\x04':
                ...
                if bytes_total[pos:pos+4] != b'\x00\x00\x00\x00':
                    # truncated message ?
                    LOG.warning("MAIO Message is truncated, stopping dissect and report anyway.")
                    return result_value
                else:
                    pos += 4
            else:
                LOG.warning("MAIO Message is truncated or invaild, stopping dissect and report anyway.")
                return result_value
            ...
            if delegate == None:
                 result_value.append(f"It is not supported function, type={dct['type']}, forward_type={handler.type}")
            else:
    ...
    def to_packet_string(self):
        ...
        return b"\x01\x02\x03\x04" + struct.pack(">H", len(datas)) + datas.encode() + b"\x00\x00\x00\x00"


class MAIOProcessRecorder:
    def __init__(self, sock, pid, workdir, systype, delay : int, program_cmd : list, sensor_with_tid, sensor_parent_pid : int = None):
        self._thread = None
    ...
    def start_record_process(self):
        def calculate_cpu_usage(pid):
            ...
            cpu_usage_percentage = (total_time / (num_processors * seconds_since_start)) * 100
        ...
        def calculate_relative_thread_cpu_usage(pid, thread_id : int):
            ...
            thread_cpu_usages = ((thread_cpu_times_end - thread_cpu_times_start) / (sum(system_cpu_times_end) - sum(system_cpu_times_start))) * 100
            ...
            try:
                ...
                for status_text in status_data:
                    if "RssShmem" in status_text:
                        shr_value = int(status_text.replace("RssShmem:", "").replace("kB", "").replace(" ", ""))

                    elif "VmRSS" in status_text:
                        res_value = int(status_text.replace("VmRSS:", "").replace("kB", "").replace(" ", ""))

                    elif "VmSize" in status_text:
                        virt_value = int(status_text.replace("VmSize:", "").replace("kB", "").replace(" ", ""))

                    if shr_value != None and \
                        res_value != None and \
                            virt_value != None:
                        all_found = True
                        break
                ...
                if all_found:
                    ...
                    # it is TID (Thread PID)
                        ...
                        cpu_usage_percentage = calculate_relative_thread_cpu_usage(self._ppid, self._pid)
                ...
                LOG.error(f"Can't read the PROC information, maybe it was terminated by OS, detail={ioe}")
                self.try_terminate()
                continue
    ...
    def start_record_hit_process(self):
        while True:
            if self._terminated:
                proc_name = "_".join(self._cmds)
                filename = f"{self.workdir}/{self._pid}_{proc_name}_{int(self._started_date)}.csv"
                LOG.info(F"The reported filepath is {filename}, pid={self._pid}, started_date={self._started_date}")
                return
            try:
                ...
                log_data = self.poll()
                LOG.debug(f"Captured data received, len={len(log_data)}, pid={self._pid}, cmds={self._cmds}")
                for result in log_data:
                    """
                    the data structure is following that:
                    {'TIMESTAMP': 1700643830.737748, 'COMMAND': 'STEM_DP_DEN 5', 'VIRT': 6861552, 'RES': 5273544, 'SHR': 5167616, '%CPU': 0.0}
                    """
                    self.record_to_data(result)

            except ConnectionAbortedError:
                LOG.error(f"The peer sensor is aborted, terminate record for PID {self._pid}, {self._cmds}, peer={self.peer_sock.getpeername()}")
                self.try_terminate()
            except ConnectionResetError:
                LOG.error(f"The peer sensor is disconnected, terminate record for PID {self._pid}, {self._cmds}, peer={self.peer_sock.getpeername()}")
                self.try_terminate()
    ...
    def start(self):
        if self._thread != None:
            LOG.error(f"You tried to execute recorder which already started! pid={self._pid}, cmds={self._cmds}")   
        else:
            if self.systype == MAIO_SYSTEM_TYPE_MONITOR:
                self._thread = threading.Thread(target=MAIOProcessRecorder.start_record_hit_process, args=(self,))
                self._thread.daemon = True
                self._thread.start()
                LOG.info(f"Stated hitting the record data of process, pid={self._pid}, cmds={self._cmds}")
            else:
                self._thread = threading.Thread(target=MAIOProcessRecorder.start_record_process, args=(self,))
                self._thread.daemon = True
                self._thread.start()
                LOG.info(f"Started generating the record data of process, pid={self._pid}, cmds={self._cmds}, threaded={self.threaded}, parent_pid={self._ppid}")  


class MAIORemoteHandler:
    def __init__(self, type : int, with_mont_addr: str = None, with_ssh = None, remote_port = MAIO_REMOTE_PORT):
...
def maio_intl_create_proc_record(handler : MAIORemoteHandler, rslt : MAIOMessage, system_type : int, delay : int):
    ...
    if len(detected_data) > 0:
        ...
        if system_type == MAIO_SYSTEM_TYPE_MONITOR:
            ...
            for pid, d in detected_data.items():
                ...
                mpr = MAIOProcessRecorder(rslt.peer, pid, sub_working_dir, system_type, delay, d['CMDS'], None, None) # not need to TID, Parent ID (just monitor)
                mpr.start()
                handler._records.append(mpr)

                for tid in d["TIDS"]:
                    mpr = MAIOProcessRecorder(rslt.peer, tid, sub_working_dir, system_type, delay, d['CMDS'], None, None) # not need to TID, Parent ID (just monitor)
                    mpr.start()
                    handler._records.append(mpr)
        else:
            for pid, d in detected_data.items():
                mpr = MAIOProcessRecorder(handler._sock, pid, None, system_type, delay, d['CMDS'], d["TIDS"])
                mpr.start()
                handler._records.append(mpr)

                for tid in d["TIDS"]:
                    mpr = MAIOProcessRecorder(handler._sock, tid, None, system_type, delay, d['CMDS'], d["TIDS"], pid)
                    mpr.start()
                    handler._records.append(mpr)
    else:
        LOG.warning(f"Catched record processes, but detected data is None, target={rslt.peer.getpeername()}")
...
def maio_intl_msg_dissect(handler, peer, bytes):
    ...
    for maio_obj in maio_object:
        ...
        if isinstance(maio_obj, str):
            LOG.error(f"It is not vaild MAIO message, byte_len={len(bytes)}, reason={maio_obj}")
            return False
...
def maio_intl_delegate_th(handler : MAIORemoteHandler, socket : socket.socket):
    ...
    msg = MAIOMessage._maio_msg_send_request_process("process_list.txt")
...
def maio_intl_run_monitor(handler : MAIORemoteHandler):
    ...
    LOG.debug(f"Created new thread for monitoring, tid={threading.get_ident()}, binded to *:{handler._bind_port}")
    ...
    if handler._with_ssh:
        try:
            ...
            try:
                ...
                handler._with_ssh.exec_command(f'cd /tmp && python3 /tmp/maio.py -s 1 --log-level DEBUG -i {handler._self_addr} --remote-port={handler._bind_port}')           
...
def maio_intl_run_sensor(handler : MAIORemoteHandler):
    """
    """
    LOG.info(f"Created new thread for client, tid={threading.get_ident()}, connecting to {handler._monitor_addr}:{handler._bind_port}")
    try:
    ...
    if maio_proc_handler.execute():
        while True:
            ...
            if rslt == None:
                ...
                LOG.debug(f"Ingress MAIO Message, type={maio_msg_print_message(rslt.type)}")
                ...
                if system_type == MAIO_SYSTEM_TYPE_MONITOR:
                    ...
                    if rslt.type == MAIO_MSG_REQUEST_PROCESS:
                        ...
                        maio_intl_create_proc_record(maio_proc_handler, rslt, MAIO_SYSTEM_TYPE_MONITOR, args.delay)
                        ...
                        if not is_detected_worker:                        
                            LOG.error(f"Unknown PID Recorder, maybe it is killed by unknown or unexpected, skiping data={result}")
                    ...
                    if rslt.type == MAIO_MSG_REQUEST_PROCESS:
                        LOG.info(f"Process record is now started, pids={rslt.data.keys()}")
                        # ready for recording each process
                        msg = MAIOMessage._maio_msg_send_report_process(rslt.data)
                        maio_intl_create_proc_record(maio_proc_handler, rslt, MAIO_SYSTEM_TYPE_SENSOR, args.delay)
                        ...
                        for r_worker  in maio_proc_handler._records:
                            if r_worker._pid == pid:
                                ...
                                maio_proc_handler._sock.send(MAIOMessage._maio_msg_send_capture_process_by_sensor(data).to_packet_string())
                                is_detected_worker = True
                                break
                        if not is_detected_worker:                        
                            LOG.error(f"Unknown PID Recorder, maybe it is killed by unknown or unexpected, skiping data={result}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--remote-port", metavar="remote_port", help="Set your remote port when sensor is connecting", type=int, default=MAIO_REMOTE_PORT)
    ...
    parser.add_argument("-d", "--delay", help="Set delay for monitoring resource", type=int, default=1)
    parser.add_argument("-s", "--system-type", help="Set sensor mode (0=monitor, 1=sensor)", type=int, default=0)
    ...
    LOG.info(f"@@@@@@ MAIO started, log_level={logging.getLevelName(log_level)}, system_type={args.system_type}")
